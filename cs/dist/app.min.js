...
function fetchShared() {
    var songlist = jQuery('#shared');
    db.ref().child('shared').orderByChild('uid').equalTo(userData.uid).on('value', function(snapshot) {
        songlist.find('.data').remove();
        shared = snapshot.val();
        for (var key in shared) {
            var info = shared[key];
            var li = jQuery('<li class="data" onclick="handleSongListClick(event);">' + '<a href="#" data-id="' + key + '"><b class="badge bg-success dker pull-right">' + (info.songs ? Object.keys(info.songs).length : '') + '</b><span>' + info.lid + '</span></a></li>');
            songlist.append(li);
        }
        loadData();
    });
...
function addShared(listID) {
    ...
    var newListKey = db.ref().child('shared').push().key;
    var updates = {};
    updates['shared/' + newListKey] = listData;
    db.ref().update(updates, function(error) {
        if (error) {
        } else {
            //updateSongListUI();
