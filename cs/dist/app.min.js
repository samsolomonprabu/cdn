var languages = [
    { id: 'arabic', label: 'Arabic (عربى)' }, // 11
    { id: 'chinese', label: 'Chinese (中文)' }, // 12
    { id: 'english', label: 'English' }, // 2
    { id: 'hindi', label: 'Hindi (हिंदी)' }, // 3
    { id: 'kamba', label: 'Kamba' }, //8
    { id: 'kannada', label: 'Kannada (ಕನ್ನಡ)' }, // 7
    { id: 'malayalam', label: 'Malayalam (മലയാളം)' }, // 5
    { id: 'nepali', label: 'Nepali (नेपाली)' }, // 6
    { id: 'russian', label: 'Russian (русский)' }, // 10
    { id: 'swahili', label: 'Swahili (Kiswahili)' }, // 9
    { id: 'tamil', label: 'Tamil (தமிழ்)' }, // 1
    { id: 'telugu', label: 'Telugu (తెలుగు)' } // 4
];

var androidAppUrls = {
    'tamil': 'https://play.google.com/store/apps/details?id=sam.songbook.tamil',
    'english': 'https://play.google.com/store/apps/details?id=sam.songbook.english',
    'hindi': 'https://play.google.com/store/apps/details?id=sam.songbook.hindi',
    'telugu': 'https://play.google.com/store/apps/details?id=sam.songbook.telugu',
    'malayalam': 'https://play.google.com/store/apps/details?id=sam.songbook.malayalam',
    'nepali': 'https://play.google.com/store/apps/details?id=sam.songbook.nepali',
    'kannada': 'https://play.google.com/store/apps/details?id=sam.songbook.kannada',
    'swahili': 'https://play.google.com/store/apps/details?id=sam.songbook.kenya',
    'kamba': 'https://play.google.com/store/apps/details?id=sam.songbook.kenya'
};

var iosAppUrls = {
    'tamil': 'https://itunes.apple.com/us/app/christian-songbook-tamil/id1465614423?ls=1&mt=8',
    'english': 'https://apps.apple.com/us/app/english-christian-songs/id1467533537?ls=1',
    'hindi': 'https://itunes.apple.com/us/app/hindi-christian-songs/id1466978879?ls=1&mt=8',
    'telugu': 'https://apps.apple.com/us/app/telugu-christian-songs/id1467876263?ls=1',
    'malayalam': 'https://itunes.apple.com/us/app/malayalam-christian-songs/id1466955404?ls=1&mt=8',
    'nepali': 'https://itunes.apple.com/us/app/nepali-christian-songs/id1460396012?ls=1&mt=8',
    'kannada': 'https://itunes.apple.com/us/app/kannada-christian-songs/id1465134271?ls=1&mt=8'
};

var buyPPTLinks = {
    'tamil' : { url: 'ATY7UKG', cost: '5,000.00' },
    'english' : { url: 'WHLzmxv', cost: '4,000.00' },
    'telugu' : { url: 'xqbYEHe', cost: '2,500.00' },
    'hindi' : { url: 'rqs2q7z', cost: '1,000.00' },
    'kannada' : { url: 'CIQhPpl', cost: '2,000.00' },
    'malayalam' : { url: 'JaqPLTx', cost: '1,500.00' },
    'nepali' : { url: 'TeICHLM', cost: '1,000.00'}
};
...
function fetchCurrentLanguage() {
    ...
    if(HTTP_GET['lang']) {
        ...
        currentLanguage === 'null' && localStorage.removeItem('lang');
        if(currentLanguage) {
...
function loadData() {
    ...
    if (page === 'index.php' || page === '') {
        ...
        genCategories(albums, 'albums.php', 'handleAlbumClick(event);', 'https://ik.imagekit.io/pukqq5vmw/albums/' + currentLanguage + '/');
        data = [];
        HTTP_GET['id'] && filterAlbum();
    } else if (page === 'artists.php') {
        genCategories(artists, 'artists.php', 'handleArtistClick(event);', 'https://ik.imagekit.io/pukqq5vmw/artists/' + currentLanguage + '/');
        data = [];
        HTTP_GET['id'] && filterArtist();
    } else if (page === 'genres.php') {
        genCategories(genres, 'genres.php', 'handleGenreClick(event);', 'https://ik.imagekit.io/pukqq5vmw/genres/' + currentLanguage + '/');
        data = [];
        HTTP_GET['id'] && filterGenre();
    ...
    albums.length == 0 ? jQuery('#albumNav').hide() : jQuery('#albumNav').show().find('.songs-count').text(albums.length);
    artists.length == 0 ? jQuery('#artistNav').hide() : jQuery('#artistNav').show().find('.songs-count').text(artists.length);
    genres.length == 0 ? jQuery('#genreNav').hide() : jQuery('#genreNav').show().find('.songs-count').text(genres.length);
    videoSongs.length == 0 ? jQuery('#videoNav').hide() : jQuery('#videoNav').show().find('.songs-count').text(videoSongs.length);
    karaokeSongs.length == 0 ? jQuery('#karaokeNav').hide() : jQuery('#karaokeNav').show().find('.songs-count').text(karaokeSongs.length);
    jQuery('#allSongsEl .songs-count').text(songs.length);
...
function fetchSongsList() {
    if (!songlistData[HTTP_GET['id']]) {
        addShared(HTTP_GET['id']);
    } else {
...
function genCategories(categories, link, handler, imgBaseURL) {
    ...
    categories.forEach(function(item, index) {
        ...
        category.find('.data-category-link').attr('href', link + '?id=' + item.id + '&title=' + item.title).attr('onclick', handler).attr('data-index', index);
...
function genSongsCount() {
    ...
    genres.forEach(function(genre, index) {
        genre.count = langSongs.filter(function(song) {
            return song.genre == genre.id;
        }).length;
    });
};
...
function constructAlphabets() {
    ...
    var page = window.location.pathname.split('/').pop() + '?';
    if(page.indexOf('album') !== -1 || page.indexOf('artist') !== -1 || page.indexOf('genre') !== -1) {
        page += 'id=' + HTTP_GET['id'] + '&';
    } 
...
function handleAlphabetClick(event) {
    ...
    HTTP_GET = getQueryParams(window.location.search);
    var page = window.location.pathname.split('/').pop();
    if(page === '' || page === 'index.php' || page === 'albums.php' || page === 'artists.php' || page === 'genres.php') {
        fetch();
...
function handleGenreClick(event) {
    var el = jQuery(event.currentTarget);
    var index = el.attr('data-index');
    window.history.pushState('genre' + index, el.text(), el.attr('href'));
    HTTP_GET = getQueryParams(window.location.search);
    filterGenre();
    constructAlphabets();
    fetch();
...
function handleVideoClick(event) {
    ...
    document.title = el.attr('data-title') + ' | ' + capitalize(fetchCurrentLanguage()) + ' Christian Song Book | Part of APA Mission';
...
function filterAlbum() {
    ...
    document.title = albumInfo.title + ' | ' + capitalize(fetchCurrentLanguage()) + ' Christian Song Book | Part of APA Mission';
};
...
function filterArtist() {
    ...
    document.title = artistInfo.title + ' | ' + capitalize(fetchCurrentLanguage()) + ' Christian Song Book | Part of APA Mission';
};

function filterGenre() {
    var genreId = HTTP_GET['id'];
    data = langSongs.filter(function(item) {
        return item.genre == genreId;
    });
    genreInfo = genres.filter(function(item) {
        return item.id == genreId;
    })[0];
    jQuery('#categoryTitle').text(genreInfo.title);
    document.title = genreInfo.title + ' | ' + capitalize(fetchCurrentLanguage()) + ' Christian Song Book | Part of APA Mission';
};
...
function fetch() {
    ...
    for(var i = start - 1; (i < (start + batchSize - 1) && i < filtered.length); i++) {
        ...
        var songURL = url + (url.indexOf('.php') != -1 ? '' : 'index.php') + 'song=' + md5(filtered[i].id) + '&title=' + filtered[i].title + '&lang=' + fetchCurrentLanguage() + '&id=' + filtered[i].id;
...
function fetchSong(showYoutube) {
    ...
    document.title = HTTP_GET['title'] + ' | ' + capitalize(fetchCurrentLanguage()) + ' Christian Song Book | Part of APA Mission';
    var canvasCont = jQuery('#canvasContainer');
    canvasCont.html('');
    jQuery('#lyricStatus').show().text('Loading song...');
    jQuery('#downloadOptions').hide();
    jQuery('#downloadOptions a').eq(0).attr('href', 'https://downloads.christiansongbook.in/index.php?id=' + HTTP_GET['song']);
    jQuery('#downloadOptions a').eq(1).attr('href', 'https://downloads.christiansongbook.in/export.php?type=pdf&id=' + HTTP_GET['song']);
    jQuery('#downloadOptions a').eq(2).attr('href', 'https://downloads.christiansongbook.in/export.php?type=image&id=' + HTTP_GET['song']);
    jQuery('#songLink').attr('href', 'index.php?lang=' + HTTP_GET['lang'] + '&song=' + HTTP_GET['song'] + '&title=' + HTTP_GET['title']);
    jQuery.ajax({
        url: 'get-song.php',
        data: {
            id: HTTP_GET['song']
...
function init() {
    ...
    var page = window.location.pathname.split('/').pop();
    if(page === '' || page === 'index.php' || page === 'albums.php' || page === 'artists.php' || page === 'genres.php' || page === 'songlist.php') {
        fetch();
...
function nextSlide() {
    ...
    if(index < count) {
        ...
        gotoStanza(index); // Sends signal to cast receiver device if connected
    }
};
...
function handleLangClick(event, lang) {
    ...
    jQuery('#genreNav a').attr('href', 'genres.php?lang=' + lang);
...
// Sets android and iOS app links
function setAppDownloadUrls() {
    var lang = fetchCurrentLanguage();
    if(androidAppUrls[lang]) {
        jQuery('.android-app').show().find('a').attr('href', androidAppUrls[lang]);
    } else {
        jQuery('.android-app').hide();
    }
    if(iosAppUrls[lang]) {
        jQuery('.ios-app').show().find('a').attr('href', iosAppUrls[lang]);
    } else {
        jQuery('.ios-app').hide();
    }  
};

function setPPTDownloadUrls() {
    var lang = fetchCurrentLanguage();
    var info = buyPPTLinks[lang];
    var pptContainer = document.getElementById('buyPPTCont');
    if (info) {
        var rURL = 'https://rzp.io/l/';
        if (pptContainer) {
            pptContainer.querySelector('a').setAttribute('href', rURL + info.url);
            pptContainer.querySelector('#pptCost').innerText = info.cost;
            pptContainer.style.display = '';
        }
        jQuery('.download-ppt').show().find('a').attr('href', rURL + info.url);
    } else {
        pptContainer.style.display = 'none';
        jQuery('.download-ppt').hide();
    }
};
...
initializeCastApi = function() {
    ...
    jQuery(document.getElementById('castbutton').shadowRoot.childNodes[1]).find('path').attr('style', 'fill:#ffffff;');
};
...
function gotoStanza(index) {
    var castSession = cast.framework.CastContext.getInstance().getCurrentSession();
    if(castSession){
        castSession.sendMessage('urn:x-cast:sam.songbook.tamil.goto', {
            currentSlide: index
        });
    }
}
...
function fetchShared() {
    var listIds = [];
    db.ref().child('shared').orderByChild('uid').equalTo(userData.uid).on('value', function(snapshot) {
        shared = snapshot.val();
        for (var key in shared) {
            listIds.push(key.split('***')[1]);
        }
    });

    var sharedListCont = jQuery('#shared');
    sharedListCont.find('.data').remove();
};

function addShared(listID) {
    var newListKey = userData.uid + '***' + listID;
    var updates = {};
    updates['shared/' + newListKey] = { uid: userData.uid };
    db.ref().update(updates, function(error) {
        if (error) {
        } else {
            //updateSongListUI();
...
jQuery(function() {
    setTimeout(function() {
        console.clear();
        console.log('%c Stop !', 'color: red; font-weight: bold; font-size: 80px;text-shadow: -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000;');
        console.log('%c This is a browser feature intended for developers.', 'font-weight: bold; font-size: 30px;');
    }, 5000);
});
